      ...
      <form action="" id="add-task-form">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <div class="form-check form-check-inline mb-1">
                <input class="form-check-input" type="checkbox" name="content_pillar[]" value="<?= $pillar ?>" 
                      <?php if($pillar === 'Lainnya'): ?>id="lainnyaCheckbox"<?php endif; ?>>
                <label class="form-check-label"><?= $pillar ?></label>
            </div>
            <?php endforeach; ?>
            
            <!-- Hidden text input for Task Type Lainnya -->
            <div id="lainnyaInputContainer" class="mt-2" style="display: none;">
                <label for="lainnyaText">Please specify:</label>
                <input type="text" class="form-control w-50" id="lainnyaText" name="lainnya_text" placeholder="Enter other task type...">
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            ?>
            <div class="form-check form-check-inline mb-1">
                <input class="form-check-input platform-checkbox" type="checkbox" name="platform[]" value="<?= $plat ?>" 
                      <?php if($plat === 'Lainnya'): ?>data-lainnya="platform"<?php endif; ?>>
                <label class="form-check-label"><?= $plat ?></label>
            </div>
            <?php endforeach; ?>
            
            <!-- Hidden text input for Platform Lainnya -->
            <div id="platformLainnyaInputContainer" class="mt-2" style="display: none;">
                <label for="platformLainnyaText">Please specify platform:</label>
                <input type="text" class="form-control w-50" id="platformLainnyaText" name="platform_lainnya_text" placeholder="Enter other platform...">
            </div>
        </div>
...
$(document).ready(function(){
    ...
    // Handle Task Type Lainnya checkbox
    $('input[name="content_pillar[]"][value="Lainnya"]').change(function() {
        if ($(this).is(':checked')) {
            $('#lainnyaInputContainer').show();
            $('#lainnyaText').prop('required', true);
        } else {
            $('#lainnyaInputContainer').hide();
            $('#lainnyaText').prop('required', false).val('');
        }
    });
    
    // Handle Platform Lainnya checkbox - Using attribute selector
    $('input[name="platform[]"][value="Lainnya"]').change(function() {
        if ($(this).is(':checked')) {
            $('#platformLainnyaInputContainer').show();
            $('#platformLainnyaText').prop('required', true);
        } else {
            $('#platformLainnyaInputContainer').hide();
            $('#platformLainnyaText').prop('required', false).val('');
        }
    });
    ...
    $('#add-task-form').off('submit').submit(function(e){
        ...
        // Validate Task Type Lainnya
        if($('input[name="content_pillar[]"][value="Lainnya"]').is(':checked') && $('#lainnyaText').val().trim() === '') {
            alert_toast("Please specify the task type for 'Lainnya'", 'warning');
            $('#lainnyaText').focus();
            return false;
        }
        
        // Validate Platform Lainnya
        if($('input[name="platform[]"][value="Lainnya"]').is(':checked') && $('#platformLainnyaText').val().trim() === '') {
            alert_toast("Please specify the platform for 'Lainnya'", 'warning');
            $('#platformLainnyaText').focus();
            return false;
        }
