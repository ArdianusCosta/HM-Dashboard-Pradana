...
if ($login_type == 2) { 
    ...
    // Member or client: only projects where they are listed
    $where_role = " FIND_IN_SET($current_user_id, p.user_ids) ";
} else {
    // Admin (Role 1) or others
    $where_role = " 1=1 "; 
}
...
if ($result && mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        if (!empty($row["end_date"])) {
            ...
            // so add one day to ensure the due date itself is included.
            $endInclusive = date('Y-m-d', strtotime($row["end_date"] . ' +1 day'));
            $data[] = array(
                ...
                'start'         => $row["end_date"], 
                'end'           => $endInclusive,
                'allDay'        => true, // send as all-day to avoid timezone shifts
